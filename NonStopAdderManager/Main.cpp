...
int _tmain(int argc, TCHAR *argv[]) {
  ...
  HANDLE handles[] = { pi1.hProcess, pi2.hProcess };
  WaitForMultipleObjects(sizeof(handles) / sizeof(HANDLE), handles, TRUE,
                         INFINITE);
  ...
  if (return_val1 == -1 || return_val2 == -1) return -1;

  sum += return_val1;
  sum += return_val2;

  _tprintf(_T("ret 1 = %d, ret 2 = %d \n"), return_val1, return_val2);
  _tprintf(_T("total: %d \n"), sum);
